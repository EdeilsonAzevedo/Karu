...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=2,
            help='Número de alertas para disparar'
        )
    ...
    def handle(self, *args, **options):
        ...
        warning_signs = ClinicalWarningSign.objects.filter(
            is_present=True
        ...
        for warning_sign in warning_signs:
            self.stdout.write(
                f"→ {warning_sign.record.patient.first_name}: "
                f"{warning_sign.get_type_display()}"
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ {warning_signs.count()} alertas enviados para a fila!"
            )
        )
