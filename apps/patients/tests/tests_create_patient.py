...
class PatientCreateViewTests(TestCase):
    ...
    def test_post_valid_data_creates_objects_and_redirects(self):
        ...
        self.assertEqual(
            ...
            302,
            f"O formulário não foi validado. Erros: {response.context.get('patient_form').errors if response.context else 'No context'}",
        )
        ...
        self.assertTrue(
            ClinicalEvaluation.objects.filter(
                record=record, type="pediatric", status="normal"
            ).exists()
        )
        self.assertTrue(
            InterdisciplinaryEvaluation.objects.filter(
                record=record, area="nursing", notes="Orientações de amamentação realizadas."
            ).exists()
        )
    ...
    def test_post_invalid_data_rerenders_form_with_errors(self):
        ...
        """
        form_data = self.valid_base_data.copy()
        form_data["date_of_birth"] = (datetime.date.today() + datetime.timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
