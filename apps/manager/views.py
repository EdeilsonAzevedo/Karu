...
MICRORREGIAO_POR_MUNICIPIO = {
    normalize_str(k): v for k, v in MICRORREGIAO_POR_MUNICIPIO.items()
}
...
def dashboard_stats_api(request):
    ...
    for patient in all_patients:
        ...
        if sinal_count >= 2:
            ...
            sign = latest_consultation.warning_signs.filter(is_present=True).first() if latest_consultation else None
